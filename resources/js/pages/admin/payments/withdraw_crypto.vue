    ...
    <div class="panel">
      ...
      <table class="table table-sm table-bordered table-hover table-striped">
        ...
        <tr v-for="withdraw in withdrawals.data">
          ...
          <td>{{ withdraw.address }}
            <div v-if="withdraw.dest_tag"><small>{{ $t('withdraw.dest_tag')}}: {{ withdraw.dest_tag }}</small></div>
          </td>
          <td>{{ withdraw.created_at | moment }}</td>
          <td>{{ $t(withdraw.status) }}</td>
          <td>
            <button v-if="withdraw.status == 'waiting' && withdraw.withdrawable" @click="confirmModal(withdraw, true)" class="btn-sm btn-outline-success">{{
              $t('approve')}}
            </button>
            ...
            <span v-if="withdraw.status == 'waiting' && !withdraw.withdrawable" class="badge badge-danger" :title="getEvaluator(withdraw)">{{ $t('admin.exchange_settings.currencies.withdraw_insufficient_funds')}}</span>
